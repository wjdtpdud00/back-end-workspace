 ...
 * - 인터페이스는 interface 키워드로 정의도이ㅓ 있고 인터페이스 내에 변수, 메서드 생성 불가능
 * 
 * extends와 implements
 * - 클래스 간의 상속 관계일 때: 자식 클래스명 extends 부모 클래스명
 * - 클래스와 인터페이스의 구현 관계일 때 : 구현 클래스명 implements 인터페이스명, 인터페이스명, ...
 * - 인터페이스 간의 상속 관계일 때 : 인터페이스명 extends 인터페이스명, 인터페이스명...(다중상속 가능)
 * 
 * */
...
//		Vehicle v = new Vehicle();// <-- 인터페이스 객체 생성 X
		Vehicle bus = new Bus();// 참조변수는 사용 가능
		Vehicle taxi = new Bus();// 참조변수는 사용 가능

 ...
 * 
 * *Stream API
 * 	- 스트림은 자바 8부터 추가된 기능으로 컬렉션(배열)의 저장 요소들을 하나씩
 *    참조해서 람다식으로 처리할 수 있도록 해주는 반복자
...
//		c.method1();
	...
	public void method1() {
		// 첫 번째 매개 값 ~ 두 번째 매개 값 이전까지의 값을 요소로 가지는
		// 스트림 객체 생성 하기!
		//[range]
		IntStream stream = IntStream.range(1, 10);// 1~9까지 담고있음
		int sum = stream.sum();
		System.out.println("range의 sum: " + sum);
		
		// 첫번째 매개값 ~ 두 번째 매개 값까지의 값을 요소로 가지는 
		// 스트림 객체 생성
		//[rangeClosed]
		stream = IntStream.rangeClosed(1, 10);// 1~10까지 담고있음
		sum = stream.sum();

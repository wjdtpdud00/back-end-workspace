...
SELECT dept_code,
       ...
       min(salary) "최저 급여",
       max(salary) "최고 급여"
-- 집계 함수 또는 본인만 select에 올 수 있음 
FROM employee
GROUP BY dept_code
-- ORDER BY dept_code;
ORDER BY dept_code; /*또는 1*/
...
/*
	rollup|cube(칼럼1, 칼럼2) (cube는 MYSQL에서 사용안함) - 실제 볼일은 없음.. 집계 함수
    - 그룹별 산출한 결과 갑의 중간 집계를 계산 해주는 함수
    ...
    저희 문제에도 안나옴!! NEVER !!몰라도됨!!!
*/
-- 직급별 급여 합 조회
SELECT dept_code, job_code, sum(salary), grouping(dept_code)  
FROM employee
...
/*
    ...
    --합집합 
    UNION : 두 쿼리문을 수행한 결과값을 하나로 합쳐서 추출(중복되는 행 제거)
    UNION ALL : 두 쿼리문을 수행한 결과값을 하나로 합쳐서 추출(중복되는 행 제거 X)
    --교집합
    INTERSECT : 두 쿼리문을 수행한 결과값에 중복된 결과값만 추출(MYSQL X)
    -차집합
    MINUS :  선행 쿼리문의 결과값에서 후행 쿼리문의 결과값을 뺀 나머지 결과값만 추출MYSQL X)
...
SELECT emp_id, emp_name, dept_code, salary
FROM employee
WHERE dept_cide = 'D5' or salary > 3000000;

...
FROM employee, job
WHERE job_code = jpb_code; -- (오류에서 확인)ambiguous : 애매한, 모호한

-- 해결방법 1) 테이블명을 이용하는 방법 : 정확한 조회명, 어디테이블에서 가져올건지 표시.
SELECT  emp_id, emp_name, job.job_code, job_name
FROM employee, job
WHERE employee.job_code = job.job_code;

-- 해결방법2) 테이블에 별칭을 부여해서 이용하는 방법 : FROM에서 별칭 적용함.
SELECT  emp_id, emp_name, job.job_code, job_name
...
WHERE e.job_code = j.job_code
 AND job_name = '대리';
-- > where은 조건이 또 들어가면 AND,OR을 써줘야함.
...
-- >> WHERE 구문으로 풀기
SELECT dept_title, emp_id, emp_name, bonus, dept_title
FROM employee, department
...
SELECT*FROM department; -- dept_id, dpept_title
-- >> WHERE 구문으로 풀기
...
-- employee를 기준으로 넣었기 때문에, employee쪽의 내용에 null이있어도 나온다.

-- 2) RIGHT [OUTER] JOIN : 두 테이블 중 오른편에 기술된 테이블을 기준으로 JOIN
SELECT emp_name, salary, dept_title, salary*12
FROM employee RIGHT JOIN department ON (dept_code =  dept_id);
-- department를 기준으로 넣었기 때문에, department쪽의 내용에 null이있어도 나온다.

-- 3) FULL [OUTER] JOIN : 두 테이블이 가진 모든 행을 조회할 수 있음 - MYSQL에서 사용못함X
SELECT emp_name, salary, dept_title, salary*12
FROM employee FULL JOIN department ON (dept_code =  dept_id);

/*
	3. 비등가 조인 (NON EQUAL JOIN)---> 거의 쓰이지 않음.
    -  매칭시킬 컬럼에 대한 조건 작성시 '='(등호)를 사용하지 않는 조인문
    - 값의 범위에 포함되는 햄들을 연결하는 방식임.
    - ANSI 구문으로는 JOIN ON만 사용 가능! (USING 사용 불가 )
*/
SELECT * FROM employee; -- salary
SELECT * FROM sal_grade; -- 굽여 등급 테이블 : sal_level, min_sal, max_sal - salary와 연관있음.

-- 사원명, 급여, 급여 레벨 조회
-- >> ANSI 구문
SELECT emp_name, salary, sal_level
FROM employee JOIN sal_grade ON (min_sal <= salary AND salary <= max_sal); --  between and 연산자로 가능
...
SELECT e.emp_id "사원사번", e.emp_name"사원명", e.dept_code"사원부서코드", 
	   m.manager_id"사수사번", m.emp_name"사수명", m.dept_code"사수부서코드"
...
/*
    ...
    - 조인되는 모든 테이블이 각 행들이 서로서로 모두 매핑된 데이터가 검색된다.(곱집합)
...
-- 사번, 사원명, 부서명, 직급명, 지역명 조회, 국가명 조회, 급여등급 조회 (employee, department, location, national)
SELECT * FROM employee ; -- emp_id, emp_name, dept_code,  job_code
SELECT * FROM department ; -- dept_title,     dept_id, location_id, job_code
SELECT * FROM location; -- loal_name                   location_id           national_code
...
-- >> ANSI 구문으로 해보기
SELECT emp_id, emp_name, dept_title, local_name, national_name, job_name, sal_level
FROM employee
	JOIN department ON (dept_code = dept_id)
...
WHERE e.job_code = j.job_code
    ...
    AND job_name = '대리'
    AND local_name LIKE 'ASIA%';
SELECT*FROM employee;  -- emp_id, emp_name, emp_no, salary, job_code, dept_code
SELECT*FROM job; -- job_name,                       job_code
SELECT*FROM department; -- dept_title, location_id           , dept_id
SELECT*FROM location;  --              local_code, local_name
...
OR national_name = '일본'; -- WHERE national_name IN ('한국', '일본');
...
SELECT dept_title, format(avg(salary), 0) "평균 급여" -- format :소수점 몇자리까지 할건지 표시해줌.
FROM employee
	LEFT JOIN department ON(dept_id = dept_code) -- 사원들이 다 나와야 하고, employee가 왼쪽에 있으니 left임.
GROUP BY dept_code; -- @ 부서별로 나눠줌. GROUPBY는 SELECT에 컬럼들,그룹함수만 올 수있음.

-- 6. 각 부서별 총 급여의 합이 1000만원 부서명, 급여의 합을 조회
SELECT dept_title,
	sum(salary) "급여 합계"
FROM employee
	JOIN department ON (dept_id = dept_code)
...
FROM employee
	JOIN job USING (job_code)
    JOIN sal_level ON (salary BETWEEN min_sal AND max_sal);
-- >> if대신 case로 풀기
SELECT emp_id, emp_name, job_name, sal_level,
	     CASE
...
SELECT * FROM employee; -- emp_name,  salary, job_code
SELECT * FROM job; -- job_name,               job_code

-- >> ANSI 구문으로 풀기
SELECT emp_name, salary, job_name, bonus
FROM employee
	JOIN job USING (job_code)
WHERE bonus IS NULL;
...
SELECT * FROM employee; -- emp_name,  salary, job_code , dept_id
SELECT * FROM department; -- dept_title               ,dept_code, location_id
SELECT * FROM job; -- job_name,               job_code
...
FROM employee
    ...
    JOIN department ON (dept_id = dept_code)
    JOIN location ON (location_id = local_code);
-- WHERE dept_title is not null; -- >> 이미 조인할때 null도 합쳐져 있어서 필요없음.
...
FROM employee e, job h, department
WHERE e.job_code = j.job_code

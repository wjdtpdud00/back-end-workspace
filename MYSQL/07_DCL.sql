/*
	보안 및 권환 관리를 위해 root 계정을 직접 사용하지 않는 것을 권장
*/
-- 사용자 계정 생성
CREATE USER test IDENTIFIED BY 'test';
CREATE USER test2@localhost IDENTIFIED BY 'test2';
-- localhost는 본인 컴퓨터를 지칭함.
-- 사용자 계정 삭제
DROP USER test;
DROP USER test2@localhost;
-- 현재 사용자에게 부여된 권한 목록 출력 (sys 데이터 베이스 한해서)
SHOW PRIVILEGES;
...
-- 데이터베이스(스키마) 권한 회수
-- REVOKE ALL PRIVILEGES ON *.* FROM test;(오류뜬거 아님)
/*
    ...
    * 스키마(Schema) : 데이터베이스 내에 특정 테이블, 뷰, 프로시저, 함수 등을 그룹화 
    ...
    --> 데이터 베이스와 스키마를 같은 의미로 사용을 하지만, 
		몇몇 DBMS(데이터베이스 시스템)에서는 다르게 해석됨.
        MYSQL에서는 대부분 두 용어를 같이 사용!
*/
-- 스키마 생성
CREATE SCHEMA sample;

-- 스키마 삭제(오른쪽 마우스 클릭 > DROP Schemas도 가능)

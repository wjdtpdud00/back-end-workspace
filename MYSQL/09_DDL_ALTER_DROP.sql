...
AS SELECT * FROM kh.department; -- 서브쿼리를 거의 쓸일 없음X
...
ALTER TABLE dept_copy MODIFY dept_id INT; -- 오류발생! -- dept_id에 더큰 값이 이미 존재하기 때문임.
ALTER TABLE dept_copy MODIFY cname INT; --  오류 없음! -- 기존 데이터가 없으면 자료형 변경이 가능

-- dept_title의 자료형을 VARCHAR(5)로 변경
ALTER TABLE dept_copy MODIFY dept_title VARCHAR(5); -- 오류발생!
...
ALTER TABLE dept_copy DROP COLUMN dpet_title;
ALTER TABLE dept_copy DROP COLUMN location_id;
ALTER TABLE dept_copy DROP COLUMN cname;
ALTER TABLE dept_copy DROP COLUMN lname; -- 삭제 불가능!

-- member_grade 테이블의 grade_code를 삭제!
ALTER TABLE member_grade DROP COLUMN grade_code; -- 오류뜸!
...
/*
    ...
    ALTER TABLE dept_copy REMANE COLUMN 기존컬럼명 TO 변경할 컬럼명;
*/
-- dept_copy 테이블에서 lname 컬럼명을 location_name으로 변경
ALTER TABLE dept_copy RENAME COLUMN lname TO locaiton_name;
/*
	2. 제약조건 추가 / 삭제
    2-1. 제약조건 추가 (ADD)
...
FROM inforamtion_schema.TABLE_CONSTRAINTS
...
DESC dept_copy; -- DESC는 데이터에 관한 정보를 준다.
...
-- 만약 삭제하고 싶다면 자식테이블 먼저 삭제한 후 부모테이블을 삭제하면 됨!
DROP TABLE member;
DROP TABLE member_grade;

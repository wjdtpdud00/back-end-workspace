...
SELECT * FROM employee; -- emp_id, emp_name, salary, dept_code, salary
...
FROM employee
    ...
    JOIN location ON (locaton_id = local_code)
	JOIN national USING (national_code)
WHERE natonal_name = '한국';
...
SELECT * FROM vw_employee; -- kh 스키마에 view테이블에 저장됨.
...
/*
	2. 뷰 컬럼에 별칭 부여
    - 서브퀴리의 SELECT절에 함수식이나 산술연산식이 기술되어 있을 경우 반드시 별칭 지정!
*/
...
/*
	3. VIEW를 이용해서 DML(INSERT, UPDATE, DELETE) 사용
		- 뷰룰 통해서 조작하게 되면 실제 데이터가 담겨있는 베이스테이블에 반영됨
*/
...
-- 부서별 급여의 합계, 평균을 조회 SELECT 문을 vw_groupdept 뷰를 정의
...
-- employee 테이블로 job_code만 중복없이 조회한 SELECT문을 vw_dt_job 뷰 정의하시오.
...
-- 5. VIEW 옵션
-- WITH CHECK OPTION : 서브쿼리에 기술된 조건에 부합하지 않는 갑으로 수정시 에러 발생!
-- 급여가 3000000원 이상인 사원들만 조회한 SELECT 문을 vw_emp뷰 정의
...
UPDATE vw_emp
SET salary = 2000000
WHERE emp_id = 200;   -- > CHECK OPTION fail은 서브쿼리 조건부합을 하지 못하여 발생,
				      -- WITH CHECK OPTION;을 주석처리한상태로 다시 실행시켜보기!
